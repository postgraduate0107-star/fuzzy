...
| IF 污泥 (Dirt) | AND 油污 (Grease) | THEN 清洗時間 (Time) |
| :------------- | :---------------- | :------------------- |
| SD (少)        | NG (無)           | VS (很短)            |
| SD (少)        | MG (中)           | S (短)               |
| SD (少)        | LG (多)           | M (中)               |
...
-   **過程**：對於每一條規則，系統會取其前提條件（`IF` 部分）中所有輸入隸屬度的**最小值 (MIN)**，這個值代表了該條規則被觸發的「強度」。例如，如果 `污泥隸-MD` 為 0.8，`油污隸-LG` 為 0.5，則 `IF MD AND LG THEN L` 這條規則的強度就是 `MIN(0.8, 0.5) = 0.5`。
